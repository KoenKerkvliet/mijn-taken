...
/** Wat de taken samen aan tijd kosten, in minuten. Boven een kolom open werk
 *  is dat wat er nog moet; boven afgevinkt werk wat je al gedaan hebt. */
export function totaleDuur(taken: Pick<Task, 'duration_minutes'>[]): number {
  return taken.reduce((som, t) => som + (t.duration_minutes ?? 0), 0)
}
